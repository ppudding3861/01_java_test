...
public class Application02 {
     ...
     * <?> : 제한없음
     * <? extends Type> :  와일드카드의 상한제한(type과 Type의 후손을 이용해서 생성한 인스턴스만 인자로 사용가능)
     * <? super Type> :  와일드카드 하한 제한(Type 과 Type의 부모를 이용해 생성한 인스턴스만 인자로 사용가능)
     * */

        WildCardFarm wildCardFarm = new WildCardFarm();
        // 농장 생성 자체가 불가능한 것은 매개변수로 사용할 수 없다.
// AnyType의 조건은 없다 하지만 RabbitFarm의 조건은 <T extends Rabbit>으로 RabbitFarm을 생성할 수 없다는 문제가 있따.
...
//        wildCardFarm.anyType(new RabbitFarm<Reptile>(new Reptile()));
        ...
        // RabbitFarm을 생성하는 부분에는 문제가 없다. 그러나 extendsType의 매개변수 조건을 확인하면 <? extends Bunny가 있으며
